...
impl SyncBarrier {
    ...
    pub fn child_view(&self) -> ChildBarrier {
        ChildBarrier {
            c2p_write: self.c2p[1]
                .try_clone()
                .expect("failed to clone c2p write fd"),
            p2c_read: self.p2c[0]
                .try_clone()
                .expect("failed to clone p2c read fd"),
        }
    }
    pub fn parent_view(&self) -> ParentBarrier {
        ParentBarrier {
            c2p_read: self.c2p[0]
                .try_clone()
                .expect("failed to clone c2p read fd"),
            p2c_write: self.p2c[1]
                .try_clone()
                .expect("failed to clone p2c write fd"),
        }
    }
}
...
impl ParentBarrier {
    ...
    pub fn close_child_descriptors(&self) -> Result<()> {
        Ok(())
    }
}
